        ...
        for (int bs : blocksizes) {
            for (Constants.DESMode mode : Constants.DESMode.values()) {
                ...
                if (mode == ELECTRONIC_CODEBOOK) {
                    encryptECB(messages.get(0), Constants.PRIVATE_KEY,bs);
                    s = System.currentTimeMillis() - s;
                    ecb.add(s);
                } else if (mode == CIPHER_BLOCK_CHAINING) {
                    encryptCBC(messages.get(0), Constants.PRIVATE_KEY,bs);
                    s = System.currentTimeMillis() - s;
                    cbc.add(s);
                } else if (mode == CIPHER_FEEDBACK) {
                    encryptCFB(messages.get(0), Constants.PRIVATE_KEY ,bs);
                    s = System.currentTimeMillis() - s;
                    cfb.add(s);
                } else if (mode == OUTPUT_FEEDBACK) {
                    encryptOFB(messages.get(0), Constants.PRIVATE_KEY, Constants.INITIAL_VECTOR,bs);
                    s = System.currentTimeMillis() - s;
                    ofb.add(s);
                } else if (mode == COUNTER) {
                    encryptCTR(messages.get(0), Constants.PRIVATE_KEY,bs);
                    s = System.currentTimeMillis() - s;
                    cnt.add(s);
                }
                outputWriter.write(s.toString() + ' ');
